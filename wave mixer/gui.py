...
def Store_value_amplitude1(val):
    if(openflag==1):
        ...
        if amplitudescaler1 != 0:
            amplitudescaling1(wavefile1,'newwavefile1.wav')
        else:
            amplitudescaler1=1
            amplitudescaling1(wavefile1,'newwavefile1.wav')
...
def Playwavfile1():
    if(playflag==1):
        print "aaya\n"
        fout1 = wave.open("newwavefile1.wav",'rb')
        p1 = pyaudio.PyAudio()
...
def output(fout,array,fmt):
    q_data1=struct.pack(fmt,*(array))
    q=wave.open(fout,'w')
    q.setframerate(sample_rate)
    q.setnframes(num_frames)
    q.setsampwidth(sample_width)
    q.setnchannels(num_channels)
    q.writeframes(r_data1)
    q.close()
    playflag=1

def amplitudescaling1(fin,fout):
    print "aaja\n"
    global amplitudescaling1
    for i in range(0,len(integer_data1)):
        amplitudescaling1[i]=integer_data1[i]*amplitudescaler1
    print amplitudescaling
    output(fout,amplitudescaling1,fmt1)
...
Amplitude1 = Scale(Gui,label='Amplitude',orient=HORIZONTAL,length=220,width=20,sliderlength=10,from_=-50,to=50,tickinterval=20,command=Store_value_amplitude1).place(x=20,y=100)
playbutton1 = Button(text='►',fg='black',command=Playwavfile1).place(x=20,y=600)
timescaling1 = Scale(Gui,label='Time Scale',orient = HORIZONTAL,length = 220, width = 20,sliderlength = 10,from_=-10,to=10,tickinterval=2,command=Store_value_timescaling1).place(x=20,y=200)
timeshifting1 = Scale(Gui,label='Time Shift',orient=HORIZONTAL,length = 220,width=20,sliderlength=10,from_=-10,to=10,tickinterval=2,command=Store_value_timeshifting1).place(x=20,y=300)
